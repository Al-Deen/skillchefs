...
{{--                        @if (hasPermission('instructor_create'))--}}
                            <div class="align-self-center d-flex gap-2">
                                <!-- add btn -->
                                <div class="align-self-center">
                                    <a href="{{ route('admin.ambassador.create') }}" role="button" class="btn-add"
...
{{--                        @endif--}}
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            @forelse ($data['ambassadors'] as $key => $ambassador)
                                <tr>
                                    <td>{{ @$ambassador->id }}</td>
                                    <td>
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div class="product-image">
                                                <img src="{{ showImage(@$ambassador->user->image->original, 'default-1.jpeg') }}"
                                                     alt="{{ @$ambassador->user->name }}">
                                            </div>
                                            <div class="product-name ml-10">
                                                {{ Str::limit(@$ambassador->user->name, 20) }}
                                            </div>
                                        </div>
                                    </td>
                                    <td> {{ @$ambassador->enroll->count() }}</td>
                                    <td>{{ showPrice(@$ambassador->earnings) }}</td>
                                    <td>{{ showPrice(@$ambassador->balance) }}</td>
                                    <td>
                                        {{ statusBackend(@$ambassador->user->userStatus->class, $ambassador->user->userStatus->name) }}
                                    </td>

                                    <td class="create-date">{{ showDate(@$ambassador->created_at) }}</td>
...
{{--                                                @if (hasPermission('instructor_login'))--}}
                                                    <li>
                                                        <a class="dropdown-item"
                                                           href="{{ route('admin.ambassador.login', $ambassador->id) }}"><span
                                                                class="icon mr-12">
...
{{--                                                @if (hasPermission('instructor_update'))--}}
                                                    <li>
                                                        <a class="dropdown-item"
                                                           href="{{ route('admin.ambassador.edit', [$ambassador->id, 'general']) }}"><span
                                                                class="icon mr-12">
...
{{--                                                @if (hasPermission('instructor_re_activate'))--}}
                                                    <li>
                                                        <a class="dropdown-item status_update"
                                                           ...
                                                           data-href="{{ route('admin.ambassador.re_activate', $ambassador->id) }}">
...
{{--                                                @if (hasPermission('instructor_suspend'))--}}
                                                    <li>
                                                        <a class="dropdown-item delete_data" href="javascript:void(0);"
                                                           data-href="{{ route('admin.ambassador.suspend', $ambassador->id) }}">
...
{{--                                                @endif--}}
                    ...
                    <!--  table end -->
                    <!--  pagination start -->
                    @include('backend.ui-components.pagination', ['data' => $data['ambassadors']])
                    <!--  pagination end -->
                </div>
            </div>
        </div>
