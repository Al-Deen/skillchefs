    ...
    <div class="instractor-tab-widget">
        <div class="instractor-tab-widget-card">
            <div class="instractor-tab-widget-thumb">
                ...
                <a
                    href="{{ route('frontend.instructor.details', [@$data['book']->instructor->name, @$data['book']->instructor->id]) }}">
                    <img class="img-fluid" src="{{ showImage(@$data['book']->instructor->image->original) }}"
                        alt="img">
                </a>
            </div>
            <div class="d-flex flex-column align-items-center">
                <h4 class="instractor-name text-capitalize"><a
                        href="{{ route('frontend.instructor.details', [$data['book']->user->name, $data['book']->user->id]) }}">{{ @$data['book']->instructor->name }}</a>
                </h4>
...
{{--                </div>--}}
        ...
        <div class="instructor-personal-info">
            <p class="pera text-16 mb-10">{{ Str::limit(@$data['book']->instructor->instructor->about_me, 550) }}</p>
            @if (@$data['book']->instructor->instructor->experience)
                <h5 class="personal-info-title">{{ ___('frontend.Experiences') }}</h5>
                <ul class="instractor-expriences_list">
                    @foreach (@$data['book']->instructor->instructor->experience as $key => $experience)
                        <li>
